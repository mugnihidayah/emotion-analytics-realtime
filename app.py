    ...
    def recv(self, frame):
        try:
            ...
            if self.has_bbox and self.bbox is not None:
                ...
                cv2.putText(output, self.last_emotion.upper(), (x1, y1-10), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

            # Bar Engagement
            cv2.putText(output, f"Engagement: {int(self.last_score)}%", (20, 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
            
            bar_width = int((self.last_score / 100) * 200)
            cv2.rectangle(output, (20, 50), (220, 65), (50,50,50), -1) # Background bar
            cv2.rectangle(output, (20, 50), (20 + bar_width, 65), (0, 255, 0), -1) # Fill bar

            # Real-time Graphic
            cv2.putText(output, "Live Trend (30s)", (20, 90), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200,200,200), 1)
            output = self.draw_sparkline(output, list(self.score_history), 20, 100, 200, 50)
